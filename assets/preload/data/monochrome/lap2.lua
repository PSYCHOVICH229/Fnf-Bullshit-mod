...
function onCreatePost()
    makeLuaSprite('lap2','spr_lap2',480,-150)
    setObjectCamera('lap2','camHUD')
    scaleObject('lap2',1.3,1.3)
    setProperty('lap2.antialiasing',false)
    addLuaSprite('lap2',true)
end
function onSongStart()
    finishLength = songLength - 500
end
function onUpdatePost(elapsed)
    if getSongPosition() > finishLength then
        reLOAD()
        playSound('Lap2',1)
        doTweenY('ITSLAP2','lap2',50,1)
        runTimer('LAP2',3)
        finishLength = songLength -- preventing to make it loop endlessly
    end
end
function onTimerCompleted(t)
    if t == 'LAP2' then
        doTweenY('ITSLAP2','lap2',-150,1)
    end
end
function reLOAD()
    -- go back to zero 
